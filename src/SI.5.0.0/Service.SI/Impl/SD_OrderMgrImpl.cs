namespace com.Sconit.Service.SI.Impl
{
    ...
    public class SD_OrderMgrImpl : BaseMgr, ISD_OrderMgr
    {
        ...
        public Entity.SI.SD_INV.Hu DoReceiveProdOrder(string huId)
        {
            ...
            else if (!Utility.SecurityHelper.HasPermission(order.Type, order.IsCheckPartyFromAuthority, order.IsCheckPartyToAuthority, order.PartyFrom, order.PartyTo, false, false, false))
            {
        ...
        public void DoReceiveProdOrder(List<string> huIdList)
        {
            ...
            foreach (var orderMaster in orderMasters)
            {
                ...
                else if (!Utility.SecurityHelper.HasPermission(orderMaster.Type, orderMaster.IsCheckPartyFromAuthority, orderMaster.IsCheckPartyToAuthority, orderMaster.PartyFrom, orderMaster.PartyTo, false, false, false))
                {
